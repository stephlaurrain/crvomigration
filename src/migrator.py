...
class Migrator:
    ...
    def main(self):
        ...
        sqlite_conn = sqlite3.connect(f"{self.root_app}{os.path.sep}data{os.path.sep}database{os.path.sep}{self.jsprms.prms['sqlite']}")
        ...
        for table_name in tables:
            ...
            for column in columns:
                col_name = column[1]
                col_type = column[2].replace('TEXT', 'VARCHAR(4000)').replace('INTEGER', 'INT')
            ...
            # Insérer les données dans la table MariaDB
            ## placeholders = ', '.join(['%s'] * len(columns))
            ##  insert_query = f"INSERT INTO {table_name} VALUES ({placeholders})"
            ## for row in rows:
            ##     try:
            ##         mariadb_cursor.execute(insert_query, row)
            ##     except mysql.connector.Error as err:
            ##         print(f"Erreur lors de l'insertion de données dans {table_name}: {err}")
